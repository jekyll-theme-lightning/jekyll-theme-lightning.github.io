...
한편 `cover`나 `cv`, `page`는 개별 페이지를 위한 레이아웃입니다. `cover`는 인트로 페이지를 위한 레이아웃, `cv`는 약력 페이지를 제작할 때 사용하는 레이아웃입니다. `cover`와 `cv`가 빈번히 사용하는 특정 형태의 페이지를 위한 레이아웃이라면 `page` 레이아웃은 그 외의 페이지를 만들 때 사용할 수 있습니다.
